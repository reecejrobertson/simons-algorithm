...
for i in range(1, ITER+1):
    ...
    for n in range(2, N+1):
        ...
        with open(
            f'IonQAriaSimulator/n{n}i{i}e1s1.txt', 'w+' #l{level}
        ) as file:
        ...
        with open(
            f'IonQAriaSimulator/n{n}i{i}e1s0.txt', 'w+' #l{level}
        ) as file:
            file.write(str(result))
        ...
        result = job.result(sharpen=True).get_counts()
        with open(
            f'IonQAriaSimulator/n{n}i{i}e0s1.txt', 'w+' #l{level}
        ) as file:
            file.write(str(result))
        result = job.result().get_counts()
        with open(
            f'IonQAriaSimulator/n{n}i{i}e0s0.txt', 'w+' #l{level}
        ) as file:
            file.write(str(result))